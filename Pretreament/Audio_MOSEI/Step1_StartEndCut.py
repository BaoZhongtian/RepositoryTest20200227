...
if __name__ == '__main__':
    labelPath = r'D:\PythonProjects_Data\CMU_MOSEI\CMU_MOSEI_LabelsEmotions.csd'
    savePath = 'D:/PythonProjects_Data/CMU_MOSEI/Step1_StartEndCut_Emotion/'
    os.makedirs(savePath)

    labelData = h5py.File(name=labelPath, mode='r')
    # for sample in labelData['Emotion Labels/data/dZFV0lyedX4']:
    #     print(sample)
    for sampleName in labelData['Emotion Labels/data'].items():
        sampleName = sampleName[0]
        labelTicket, timeTicket = [], []
        print(sampleName)

        with open(os.path.join(savePath, sampleName + '.csv'), 'w') as file:
            for sample in labelData['Emotion Labels/data/%s/features' % sampleName]:
                labelTicket.append(sample)
            # labelTicket = numpy.reshape(numpy.argmax(labelTicket, axis=1), [-1, 1])
            for sample in labelData['Emotion Labels/data/%s/intervals' % sampleName]:
